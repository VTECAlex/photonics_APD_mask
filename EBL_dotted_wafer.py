    ...
    with nd.Cell(name="Dotted_Circle") as DottedCircle:
        ...
        for i in range(0, number_points + 1):
            if tt > phiphi and tt < pi + theta or tt > 2 * pi - theta and tt < 2 * pi - phiphi:
                # print(tt)
                nd.strt(length=dot_length, width=dot_width, layer=dotted_cicrcle_layer).put((R) * cos(tt),
                                                                                            (R) * sin(tt),
                                                                                            180 * tt / pi)
            tt = tt + step
        ...
        for i in range(0, int((l // dot_width) // 2) + 1):
            ...
            nd.strt(length=dot_width, width=dot_length, layer=dotted_cicrcle_layer).put(
                -R * cos(theta) + i * (step + dot_width - xxx), -R * sin(theta) - dot_length / 2 - 1.335)

        # for the minor flat
        for i in range(0, int((ll // dot_width) // 2)):
            step = ll / (ll / dot_width)
            nd.strt(length=dot_length, width=dot_width, layer=dotted_cicrcle_layer).put(
                sqrt(R ** 2 - (ll / 2) ** 2) - dot_length,
                -ll / 2 + dot_width / 2 + i * (step + dot_width))  # put(sqrt(R**2-(ll/2)**2), -ll/2+step+dot_length/2)
