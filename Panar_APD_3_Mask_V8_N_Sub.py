...
import Tile
from Device_Tile_V4_Planar import *
import Device_Tile_V4_Planar as DTV4
...
for layer in range(1,5):
    ...
    ######Ring of the reticle
    wafer = nd.Polygon(layer=layer_names[layer-1], points=geom.ring(radius=inner_length / 2 + ring_width, width= ring_width, N=700)).put(0, 0)
...
##################### These are the leans 
def Leans_Vertical_and_Horizontal_funct(layer_leans):
...
for i in range(4):
    Leans_Vertical_and_Horizontal_funct(i+1).put(0,0)
...
BVRFMF.put(0,-13117.093+1372.448+45+15+11663.31)
...
for j in range(10):
    ...
    for i in range(17):
        ...
        if (x_directio_length_of_all_tiles)**2+(y_directio_length_of_all_tiles)**2< (radius_of_wafer)**2: #This is x^2+y^2<r^2 so we have devices insied the r=4''/2
            ...
            Q4 = 4
            if j!=3:
                DTV4.APD_Tile_V2(extra_length, j+1, i+1,Q1, True, True).put(previous_position_for_Q_1_4 + extra_length/2 -15-328.147, pos1y_Q1 + j*step_y -1830 )
                names_and_coordinates_list.append(["R{}C{}Q{}".format(j+1, i+1,Q1),
                                                    previous_position_for_Q_1_4 + extra_length / 2 - 15 - 328.147 + 242.3 + 50,
                                                    pos1y_Q1 + j * step_y - 1830 + 1780 + 50,j+1,i+1, Q1])
                DTV4.APD_Tile_V2(extra_length, j+1, i+1,Q2, True, True).put(-previous_position_for_Q_2_3 - extra_length / 2 - 0 - 15 - 328.147, pos1y_Q1 + j * step_y - 1830)
                names_and_coordinates_list.append(["R{}C{}Q{}".format(j+1, i+1, Q2),
                                                    previous_position_for_Q_1_4 + extra_length / 2 - 15 - 328.147 + 242.3 + 50,
                                                    pos1y_Q1 + j * step_y - 1830 + 1780 + 50,j+1,i+1, Q2])
                if j<9:
                    DTV4.APD_Tile_V2(extra_length, j+1, i+1,Q4, True, True).put(previous_position_for_Q_1_4 + extra_length/2 -15-328.147, -pos1y_Q1 - j*step_y - 1830)
                    names_and_coordinates_list.append(["R{}C{}Q{}".format(j+1, i+1, Q4),
                                                    previous_position_for_Q_1_4 + extra_length / 2 - 15 - 328.147 + 242.3 + 50,
                                                    pos1y_Q1 + j * step_y - 1830 + 1780 + 50,j+1,i+1, Q4])
                    DTV4.APD_Tile_V2(extra_length, j+1, i+1,Q3, True, True).put(-previous_position_for_Q_2_3  - extra_length/2 -0 -15-328.147, -pos1y_Q1 - j*step_y - 1830)
                    names_and_coordinates_list.append(["R{}C{}Q{}".format(j+1, i+1, Q3),
                                                    previous_position_for_Q_1_4 + extra_length / 2 - 15 - 328.147 + 242.3 + 50,
        ...
        if (x_directio_length_of_all_tiles_PCM)**2+(y_directio_length_of_all_tiles)**2< (radius_of_wafer)**2:
            ...
            Q4 = 3
            if j==3:
                Tile_PCMs_V2_N_Sub.Tile_PCMs_all(extra_length, Q1, i, j).put( previous_position_for_Q_1_4_PCM , pos1y_Q1 + j*step_y )
                Tile_PCMs_V2_N_Sub.Tile_PCMs_all(extra_length, Q2, i, j).put( previous_position_for_Q_1_4_PCM, -pos1y_Q1 - j*step_y )
                Tile_PCMs_V2_N_Sub.Tile_PCMs_all(extra_length, Q3, i, j).put( -previous_position_for_Q_1_4_PCM -2580+480-1000, pos1y_Q1 + j*step_y )
                Tile_PCMs_V2_N_Sub.Tile_PCMs_all(extra_length, Q4, i, j).put( -previous_position_for_Q_1_4_PCM -2580+480-1000,- pos1y_Q1 - j*step_y )
...
nd.export_gds(filename=r'Mask_GDS/Palar_APD_3_Inch_N_Sub')
