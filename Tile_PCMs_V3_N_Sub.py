...
def Tile_PCMs_all(extra_length, Quadrant, i,j):
    ...
    with nd.Cell(name = 'One tile') as Single_Tile:
        ...
        frame_length = 3600+3100+1050
        frame_width = 4900
        fram_cut_area = 100

        Frame_for_tile.frafrafra(frame_length + extra_length , frame_width, fram_cut_area, 1).put(-frame_length/2 -extra_length/2 +3000/2+50+1050/2 , -frame_width/2)
        Frame_for_tile.frafrafra(frame_length + extra_length , frame_width, fram_cut_area, 2).put(-frame_length/2 -extra_length/2 +3000/2+50+1050/2, -frame_width/2)

        width_wvg = 1.5
        text_position_x = -extra_length/2-1345+200-120-500+500+1350 -1705+50
        text_position_y =1328.625
        text_position_y_layer_ID = -745+300+300-700-1500-145+4+2560
        text_rotation = -90
        text_height = 200
        ...
        text_rotation = 90
        text_height = 180
        layerforthetext = "Metal Rings"
        if Quadrant == 1:
            #nd.text("l{}um".format((1+extra_length/1000)), height= text_height).put(text_position_x, text_position_y,text_rotation)
            
            nd.text("R{}C{}Q{} VTEC".format((row_ID),(column_ID),(1)), height= text_height, layer=layerforthetext).put(text_position_x, text_position_y_layer_ID,text_rotation)

        elif Quadrant == 2:
            #nd.text("l{}um".format((1+extra_length/1000)), height= text_height).put(text_position_x, text_position_y,text_rotation)
            nd.text("R{}C{}Q{} VTEC".format((row_ID),(column_ID),(2)), height= text_height, layer=layerforthetext).put(text_position_x, text_position_y_layer_ID,text_rotation)

        elif Quadrant ==3:
            #nd.text("l{}um".format((1+extra_length/1000)), height= text_height).put(text_position_x, text_position_y,text_rotation)
            nd.text("R{}C{}Q{} VTEC".format((row_ID),(column_ID),(3)), height= text_height, layer=layerforthetext).put(text_position_x, text_position_y_layer_ID,text_rotation)

        else:
            #nd.text("l{}um".format((1+extra_length/1000)), height= text_height).put(text_position_x, text_position_y,text_rotation)
            nd.text("R{}C{}Q{} VTEC".format((row_ID),(column_ID),(4)), height= text_height, layer=layerforthetext).put(text_position_x, text_position_y_layer_ID,text_rotation)
        ...
        def matrix_of_bars(layer_bars):
            with nd.Cell(name = "Matrix of bars") as matrix_of_bars:
                ...
                if layer == 'Metal Rings':
                    ...
                    nd.text("Ni/Ge/Au",height= 100, layer=layer).put(270, +850-33+500, 0)
                else:
                    nd.text("Bond Pads", height= 100, layer = layer).put(190,+850-33+500)
            return matrix_of_bars
                
        Bars1 = matrix_of_bars('Metal Rings').put(-1750+150+50,-1812-238-50,0)
        Bars2 = matrix_of_bars('N Metal').put(-150+400+50,-1812-188-100,0)
        Bars3 = matrix_of_bars('Au plating ').put(1450-200-238+50+188+850+50,-50-1812-238,0)
        ...
        with nd.Cell(name = "Cover 1") as Cover_one:
            ...
            nd.strt(length = 4700-1150+1050, width  = 1105, layer = cover_one_layer).put(-1800+1150, -222.5)
            nd.strt(length = 1150, width  = 1105-265, layer = cover_one_layer).put(-1800, -222.5-265/2)


            nd.strt(length = 2250+2450+1050, width  = 2780-1105, layer = cover_one_layer).put(-1800,-50-1050+40-1105/2)
            

        Cover_one.put(0,0)
        cover_two_layer = 'SiO'
        with nd.Cell(name = "Cover 2") as Cover_two:
            nd.strt(length = 2250+2450+1050, width  = 1380+108-13+200, layer = cover_two_layer).put(-1800,-1050+40-600+225/2-50-12.5+108/2-13/2-100)
        ...
        #top_left_hole = metal_corners.Metal_corners_hole('Open Corners').put(-1000 + 115 + 200 - 50-865-10, 4279.86 + 0.14 - 200-1830+10,90)
        ...
        bot_left_hole = metal_corners.Metal_corners_hole('Open Corners').put(-1000 + 65 + 200-865-25+10, 430 - 1000 + 150-1830-200+10, 90)

        Layer_Difference('Open Corners', 'SiO', Single_Tile)
        Dark_Current_PCM_V3.PCM_Dark_Current(extra_length=0, Quadrant=Quadrant, i=1, j=1, show_name= False, show_frame = False).put(3850-15+1050,-1830)
...
nd.export_gds(filename=r'Tile PCMS V3 N Substrate')